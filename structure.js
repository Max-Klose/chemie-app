(function(){
 ...
 function zoom(factor,center=null){const p=center?point(center):{x:view.x+view.w/2,y:view.y+view.h/2},width=Math.max(140,Math.min(12000,view.w/factor)),scale=width/view.w;view={x:p.x+(view.x-p.x)*scale,y:p.y+(view.y-p.y)*scale,w:width,h:view.h*scale};render();}
 ...
 svg.addEventListener('pointerdown',e=>{if(e.button!==0)return;e.preventDefault();pointers.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY});svg.setPointerCapture(e.pointerId);if(pointers.size>=2){if(gesture)clearTimeout(gesture.timer);gesture=null;selectionBox=null;pinch=twoFingerState();render();return;}const t=target(e),p=point(e);gesture={pointer:e.pointerId,t,p,startX:e.clientX,startY:e.clientY,drag:false,long:false,saved:false,camera:{...view},scale:svg.getScreenCTM().a};if(t.kind==='atom'&&!remove&&!selecting){gesture.offset={x:m.atom(t.id).x-p.x,y:m.atom(t.id).y-p.y};gesture.timer=setTimeout(()=>{if(!gesture)return;gesture.long=true;moving=t.id;status('Bewegungsmodus: Atom ziehen, danach zum Einrasten antippen.');render();},450);}});
 svg.addEventListener('pointermove',e=>{
  if(!pointers.has(e.pointerId))return;pointers.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY});
  ...
  else if(g.drag&&g.t.kind==='field'&&!palette){view.x=g.camera.x-(e.clientX-g.startX)/g.scale;view.y=g.camera.y-(e.clientY-g.startY)/g.scale;render();}
 ...
 svg.addEventListener('pointercancel',e=>{pointers.delete(e.pointerId);if(gesture)clearTimeout(gesture.timer);gesture=null;pinch=null;selectionBox=null;render();});
